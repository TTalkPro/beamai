...
### 2.1 重入契约（真实 API 测出来的 bug，已修）

真实 MiniMax 测试暴露：`validation_turn_filter` 的重入**依赖记忆开启**。旧实现重入只传
`messages => [Feedback]`，指望工具循环的 `load_history` 把原问题和坏答案带回来——memory
开启时对（模型收到 `[Q, BadAnswer, Feedback]`），但 `memory => false` 时历史为空，模型只
收到一句「上次没通过请修正」，**原始问题丢失**，于是胡编（实测输出的是一份关于 enum 违规
的错误报告）。而 `memory => false` + 结构化抽取恰恰是最自然的组合。mock 测试抓不到——
mock 不看输入。

修法（改了 turn 链契约）：

- 工具循环 ok 结果加第 5 元 `Messages`（该跑完整消息序列）；
- turn Request 加可选 `load_history`（缺省 `true`），由 terminal 透传给循环；
- 重入改为 `messages => Messages ++ [Feedback]` + `load_history => false`——上下文完全由
  filter 重建，两种记忆配置都正确，且原问题不重复。

代价是既有 turn filter 全部要跟着改 tuple 元数。曾考虑过「一行修」的
`messages => Msgs ++ [Feedback]`（不改契约），但那样 memory 开启时原问题会重复一次并被
持久化进历史——把好的那个配置弄脏去救坏的那个，不划算。
...
| 错误反馈 | 追加到**原 user 消息**文本后重发，错误不跨重试累积 | 反馈作为新 user 消息，接在上一跑的完整消息序列后重入（见 §2.1） |
| 错误信息 | networknt 原生消息，列出期望值 | **必须列出期望值**：enum/const/pattern 的错误带上允许值/约定值/pattern 原文。只说「不合格」而不说「该是什么」，真实模型会反复给近义词直到重试耗尽——实测「晴朗」之于「晴」、partly_cloudy/fair，补上允许值后重试 3 次全败 → 1 次即中 |
