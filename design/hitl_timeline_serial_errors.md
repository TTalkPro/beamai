...
| LineageStore / timeline | `beamai_lineage_store` + `beamai_timeline` | **本次新增**：fork/rollback/lineage，ETS 优先 |
...
- **behaviour `beamai_lineage_store`** + ETS 实现（血缘记录）。**第一个适配 ETS**。
- 模块 `beamai_timeline`，deps 显式传 `#{memory := Mem, lineage := LS, pause_store => PS}`：
