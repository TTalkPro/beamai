-module(beamai_connector_zhipu).
-behaviour(beamai_connector).

%% Behaviour callbacks
-export([chat/3, embedding/3]).

-define(DEFAULT_BASE_URL, <<"https://open.bigmodel.cn/api/paas/v4">>).
-define(DEFAULT_MODEL, <<"glm-4">>).
...
embedding(Config, Texts, Opts) ->
    BaseUrl = maps:get(base_url, Config, ?DEFAULT_BASE_URL),
    ApiKey = get_api_key(Config),
    Model = maps:get(embedding_model, Config, maps:get(model, Opts, <<"embedding-2">>)),
...
get_api_key(Config) ->
    case maps:find(api_key, Config) of
        ...
        _ -> list_to_binary(os:getenv("ZHIPU_API_KEY", ""))
    end.
    ...
    Body3 = case maps:find(tools, Opts) of
        {ok, Tools} when is_list(Tools), Tools =/= [] ->
            ...
            case maps:find(tool_choice, Opts) of
                ...
                {ok, required} -> B#{tool_choice => <<"auto">>};
                _ -> B
            end;
        _ -> Body2
    end,
