...
set_response(BodyMap, Headers) ->
    application:set_env(beamai_core, fake_backend_error, undefined),
    application:set_env(beamai_core, fake_backend_body, BodyMap),
    application:set_env(beamai_core, fake_backend_headers, Headers).

%% 配置 request_meta 返回带响应头的富错误 {error, {http_error, Status, Body, Headers}}
set_error(Status, Body, Headers) ->
    application:set_env(beamai_core, fake_backend_error, {Status, Body, Headers}).
