...
chat_client_with_filters(Filters) ->
...
rag_prepend_test() ->
    ...
    try
        {ok, Agent} = beamai_agent:new(#{chat_client => chat_client_with_filters([RAG]),
                                         memory => false}),
...
around_observe_test() ->
    ...
    try
        {ok, Agent} = beamai_agent:new(#{chat_client => chat_client_with_filters([Obs]),
                                         memory => false}),
...
reentry_runs_fresh_loop_test() ->
    ...
    try
        {ok, Agent} = beamai_agent:new(#{chat_client => chat_client_with_filters([Retry]),
                                         memory => false}),
        {ok, Result, _} = beamai_agent:run(Agent, <<"go">>),
...
registration_order_is_layering_test() ->
    ...
    try
        {ok, Agent} = beamai_agent:new(#{
            chat_client => chat_client_with_filters([F(<<"outer">>), F(<<"inner">>)]),
            memory => false}),
...
resume_through_turn_chain_reentry_test() ->
    ...
    K = chat_client_with_filters([Validate]),
    try
        {ok, Agent} = beamai_agent:new(#{
...
resume_flag_probe_test() ->
    ...
    K = chat_client_with_filters([Probe]),
    try
        {ok, Agent} = beamai_agent:new(#{
            chat_client => K, memory => false,
            interrupt_tools => [#{name => <<"ask_human">>, description => <<"a">>,
...
schema_validation_reentry_test() ->
    ...
    K = chat_client_with_filters([beamai_filters:schema_validation_turn_filter(person_schema(), 2)]),
    try
        {ok, Agent} = beamai_agent:new(#{chat_client => K, memory => false}),
        {ok, Result, _} = beamai_agent:run(Agent, <<"go">>),
...
schema_validation_code_fence_off_test() ->
    ...
    K = chat_client_with_filters([beamai_filters:schema_validation_turn_filter(
...
schema_validation_exhausts_test() ->
    ...
    _ = mock_llm2(fun(_N) -> {ok, #{content => <<"{}">>, finish_reason => <<"stop">>}} end,
                  Parent),
    K = chat_client_with_filters([beamai_filters:schema_validation_turn_filter(person_schema(), 1)]),
    try
        {ok, Agent} = beamai_agent:new(#{chat_client => K, memory => false}),
        {ok, Result, _} = beamai_agent:run(Agent, <<"go">>),
...
schema_validation_reentry_no_duplicate_with_memory_test() ->
    ...
    K = chat_client_with_filters([beamai_filters:schema_validation_turn_filter(person_schema(), 2)]),
    try
        {ok, Agent} = beamai_agent:new(#{chat_client => K}),   %% 缺省 = 启用记忆
        {ok, _Result, _} = beamai_agent:run(Agent, <<"MY-QUESTION">>),
