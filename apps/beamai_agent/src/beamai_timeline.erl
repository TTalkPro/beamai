...
%%%
%%% deps 显式传入 `#{memory := Handle, lineage := Handle, pause_store => Handle}`：
%%%   - memory / lineage 必填；pause_store 可选（缺省则暂停快照不参与 fork/清理）。
%%%
...
-type deps() :: #{
    ...
    lineage := beamai_lineage_store:handle(),
...
fork(#{memory := Mem, lineage := Lin} = Deps, Source, Opts) ->
    ...
    ok = beamai_lineage_store:record(Lin, NewConvId,
...
-spec lineage(deps(), binary()) -> {ok, beamai_lineage_store:lineage_record()} | none.
lineage(#{lineage := Lin}, ConvId) ->
    beamai_lineage_store:get(Lin, ConvId).

%% @doc 沿 parent 回溯到根，返回**根在前**的祖先链 [Root, ..., Parent, ConvId]
-spec ancestry(deps(), binary()) -> [binary()].
ancestry(#{lineage := Lin}, ConvId) ->
    ancestry_acc(Lin, ConvId, [ConvId]).

%% @doc 删分支（历史 + 暂停快照 + 血缘）；有子分支时拒绝
-spec prune(deps(), binary()) -> ok | {error, {has_children, [binary()]}}.
prune(#{memory := Mem, lineage := Lin} = Deps, ConvId) ->
    case beamai_lineage_store:children(Lin, ConvId) of
        [] ->
            ok = beamai_chat_memory:mem_clear(Mem, ConvId),
            clear_pause(Deps, ConvId),
            ok = beamai_lineage_store:delete(Lin, ConvId),
            ok;
        Kids ->
...
ancestry_acc(Lin, ConvId, Acc) ->
    case beamai_lineage_store:get(Lin, ConvId) of
        {ok, #{parent := Parent}} when is_binary(Parent) ->
            ancestry_acc(Lin, Parent, [Parent | Acc]);
        _ ->
            Acc
    end.
