...
- **beamai_filter** / **beamai_filter_chain** - 洋葱式 filter（一个 filter 含 pre_chat/post_chat/pre_tool/post_tool 四个可选 hook），包裹工具执行与 LLM 调用（详见 [docs/FILTER.md](../../docs/FILTER.md)）
...
- **beamai_memory_filter** - Memory Filter（单个 filter：pre_chat 存 delta+展开历史，post_chat 存回复）
