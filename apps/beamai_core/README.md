...
- **beamai_filter** - 过滤器，用于工具调用与 LLM 请求前后的拦截
...
- **beamai_memory_filter** - Memory 过滤器（pre_chat 存 delta+展开历史，post_chat 存回复）
...
%% 历史由 Memory 过滤器按 context 的 conversation_id 管理。
...
Kernel 无状态，每次 invoke 只传单条最新消息；历史由 Memory 过滤器按
