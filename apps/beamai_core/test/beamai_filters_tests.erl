...
logging_passthrough_test() ->
    ...
    ?assertEqual({ok, <<"r">>, #{}},
                 beamai_tool_executor:invoke(K, <<"t">>, #{}, beamai_context:new())).
...
timeout_short_circuits_test() ->
    ...
    ?assertEqual({error, timeout},
                 beamai_tool_executor:invoke(K, <<"slow">>, #{}, beamai_context:new())).
...
timeout_fast_ok_test() ->
    ...
    ?assertEqual({ok, <<"quick">>, #{}},
                 beamai_tool_executor:invoke(K, <<"fast">>, #{}, beamai_context:new())).
...
approval_rejects_sensitive_test() ->
    ...
    {ok, Result, _} = beamai_tool_executor:invoke(K, <<"danger">>, #{}, beamai_context:new()),
...
approval_ignores_non_sensitive_test() ->
    K = chat_client([#{name => <<"safe">>, parameters => #{},
               ...
               [beamai_filters:approval_filter(fun(_N, _A) -> false end)]),
    ?assertEqual({ok, <<"ran">>, #{}},
                 beamai_tool_executor:invoke(K, <<"safe">>, #{}, beamai_context:new())).
...
approval_allows_when_approved_test() ->
    ...
    ?assertEqual({ok, <<"done">>, #{}},
                 beamai_tool_executor:invoke(K, <<"danger">>, #{}, beamai_context:new())).
...
safeguard_blocks_test() ->
    ...
    try
        ...
        ?assertEqual(content_filtered, beamai_chat_response:finish_reason(Resp)),
        ?assertMatch(#{safeguard := blocked}, beamai_chat_response:metadata(Resp)),
...
safeguard_passes_test() ->
    ...
    try
        ...
        ?assertEqual(<<"llm-answer">>, beamai_chat_response:content(Resp)),
...
safeguard_custom_failure_response_test() ->
    ...
    try
        {ok, Resp, _} = beamai_chat_client:invoke_chat(K, user_msg(<<"bomb">>), #{}),
        ?assertEqual(<<"NOPE">>, beamai_chat_response:content(Resp))
    after
        meck:unload(beamai_chat_model)
    end.
