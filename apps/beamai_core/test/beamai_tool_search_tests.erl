...
%% 建带检索的 ChatClient：全量注册，filter 负责裁剪广播
search_chat_client(Opts) ->
    Tools = tools(),
    {SearchTool, Filter} = beamai_tool_search:new(Tools, Opts),
    K0 = beamai_chat_client:new(#{}, [Filter]),
    K = beamai_chat_client:add_tools(K0, [SearchTool | Tools]),
    beamai_chat_client:add_chat_model(K, beamai_chat_model:create(mock, #{})).
...
no_accumulate_uses_last_only_test() ->
    with_mock(fun() ->
        K = search_chat_client(#{accumulate => false}),
        Msgs = user(<<"q">>)
            ++ search_round(<<"s1">>, [<<"get_weather">>])
            ++ search_round(<<"s2">>, [<<"send_email">>]),
...
no_tools_key_untouched_test() ->
    with_mock(fun() ->
        K = search_chat_client(#{}),
        {ok, _, _} = beamai_chat_client:invoke_chat(K, user(<<"q">>), #{}),
...
custom_tool_name_test() ->
    with_mock(fun() ->
        ...
        K0 = beamai_chat_client:new(#{}, [Filter]),
        K1 = beamai_chat_client:add_tools(K0, [SearchTool | Tools]),
        K = beamai_chat_client:add_chat_model(K1, beamai_chat_model:create(mock, #{})),
