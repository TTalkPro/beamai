...
配合 ChatMemory store 按 `conversation_id` 管理。
...
| `beamai_memory_filter` | pre_chat(-1000) 存 delta + 展开历史；post_chat(+1000) 存回复 |
| `beamai_kernel:with_memory/2` | 绑定 store 并挂载上述两个过滤器 |
...
| assistant 消息谁记录 | 本地拼接 | post_chat 过滤器存入 store |
...
└─ tool_calling_loop，delta = [User消息]
   └─ run_chat_pipeline(delta)
      ...
      ├─ LLM 调用
      └─ [post_chat +1000] memory: mem_add(s1, assistant 回复)
   ├─ 有 tool_calls → 执行工具（走 pre/post_invocation 过滤器），delta := [工具结果]，回到循环
   └─ 纯文本 → 返回 {ok, Response, Ctx}
```

system_prompts 作为临时 pre_chat(-500) 过滤器注入，位于历史展开之后、LLM 之前，
**不写入存储**。
...
| `apps/beamai_core/src/kernel/beamai_memory_filter.erl` | Memory 过滤器 |
