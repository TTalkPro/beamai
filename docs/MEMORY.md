...
filter（含单个 around_chat hook，详见 [Filter 文档](FILTER.md)）。
...
| `beamai_chat_memory_dets` | DETS 持久化实现（重启后按 conversation_id 恢复历史） |
| `beamai_chat_memory_window` | 滑动窗口包装：底层保留全量，`mem_get` 时套条数窗口 |
| `beamai_memory_filter` | 单个 filter：around_chat 前置存 delta + 展开历史、后置存回复 |
| `beamai_kernel:new/2` | 构建时把 memory filter 放进 filters 列表（首位 = 最外层） |
