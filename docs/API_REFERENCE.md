...
统一 HTTP 客户端接口。底层支持两种 backend（默认 Gun，支持 HTTP/2；备选 Hackney），通过 `set_backend/1` 切换。
...
%% 设置 HTTP backend（beamai_http_gun | beamai_http_hackney）
...
| 特性 | Gun（默认） | Hackney |
|------|-------------|---------|
| HTTP/2 | 支持（按池经 `protocols` 显式启用，见 [HTTP.md](HTTP.md)） | 不支持 |
...
HTTP 客户端行为定义。`beamai_http_gun` / `beamai_http_hackney` 均实现此 behaviour。
